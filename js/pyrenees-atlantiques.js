		...
		var carte = d3.select('svg#entete')
			...
			.on('mouseover', (d, i, nodes) => {
				...
				if (toto.offsetLeft > largeur / 2) {
					toto.style.left = toto.offsetLeft - 300 + "px";
...
function LaRepubliqueEnMarche() {
	...
	function parseData(data) {
		...
		var areaScale = d3.scaleLinear()
			...
			.range(['white', '#FBBA00']);
		//CARTE
		d3.select('svg#Renaissance')
			.attr("preserveAspectRatio", "xMinYMin meet")
			.attr("viewBox", "-15 0 670 405")
			.append('g')
			.attr('id', 'carteRenaissance')
			.selectAll('path')
			.data(data.features)
			.enter()
			.append('path')
		...
		var svg = d3.select("div#deptRenaissance")
			.append("svg")
			.attr("width", width)
			.attr("height", height)
			.attr("preserveAspectRatio", "xMinYMin meet")
			.attr("viewBox", "-70 -10 480 60")
			.attr("id", 'echelleRenaissance');
